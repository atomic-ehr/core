...
const mockClock = {
  now: () => Date.now(),
  toISOString: (timestamp?: number) => new Date(timestamp || Date.now()).toISOString()
};
...
async function demonstrateHooksSystem() {
  ...
  const timingHook = HookUtils.withTiming({
    ...
    handler: async (context: any) => {
      ...
      if (duration > 100) { // Slow request threshold
  ...
  try {
    ...
    const responseContext = ContextFactory.createResponseContext(requestContext, {
      statusCode: 201,
